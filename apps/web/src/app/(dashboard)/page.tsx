      ...
      <PageHeader
        ...
        actions={
          <nav aria-label="Quick actions" className="flex flex-wrap gap-2">
            <Link href="/patients/new" className="focus-visible:ring-primary-500 bg-primary-500 hover:bg-primary-600 active:bg-primary-700 inline-flex h-8 items-center justify-center gap-2 rounded-md px-3 text-xs font-medium text-white transition-colors focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:outline-none" aria-label="Register a new patient">
              + New Patient
            </Link>
            <Link href="/encounters" className="focus-visible:ring-primary-500 inline-flex h-8 items-center justify-center gap-2 rounded-md border border-neutral-300 bg-white px-3 text-xs font-medium text-neutral-700 transition-colors hover:bg-neutral-50 focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:outline-none active:bg-neutral-100" aria-label="Log a new encounter">
              + Log Encounter
            </Link>
            <Link href="/payments" className="focus-visible:ring-primary-500 inline-flex h-8 items-center justify-center gap-2 rounded-md border border-neutral-300 bg-white px-3 text-xs font-medium text-neutral-700 transition-colors hover:bg-neutral-50 focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:outline-none active:bg-neutral-100" aria-label="Initiate a payment">
              + Payment Intent
            </Link>
          </nav>
        }
      ...
      {highRiskPatients.length > 0 && (
        <section aria-label="High-risk patients">
          <div className="rounded-lg border border-danger-200 bg-danger-50 p-4">
            <h2 className="mb-3 text-sm font-semibold text-danger-700 flex items-center gap-2">
              <span aria-hidden="true">⚠️</span> High-Risk Patients ({highRiskPatients.length})
            </h2>
            <div className="space-y-2">
              {highRiskPatients.map((p) => (
                <Link key={p._id} href={`/patients/${p._id}?tab=risk`} className="flex items-center justify-between rounded bg-white px-3 py-2 text-sm hover:bg-danger-50 transition-colors">
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
          <RecentTable
            ...
            columns={[
              ...
              { key: 'createdAt', label: 'Registered', render: (row) => row.createdAt ? new Date(row.createdAt as string).toLocaleDateString() : '—' },
            ]}
            rows={recentPatients}
          />
          <RecentTable
            ...
            columns={[
              ...
              { key: 'createdAt', label: 'Time', render: (row) => row.createdAt ? new Date(row.createdAt as string).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : '—' },
            ]}
          ...
          <RecentTable
            ...
            columns={[
              ...
              { key: 'txHash', label: 'Tx Hash', render: (row) => row.txHash ? (
                <a href={`https://stellar.expert/explorer/testnet/tx/${row.txHash}`} target="_blank" rel="noopener noreferrer" className="text-primary-600 hover:underline focus:underline focus:outline-none" aria-label={`View transaction ${String(row.txHash).slice(0, 8)} on Stellar Explorer`}>
